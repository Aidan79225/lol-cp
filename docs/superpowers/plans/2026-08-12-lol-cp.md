...
def test_rare_stats_are_flagged(solved):
    """暴擊傷害件數遠低於門檻，必須被標記為低信賴。

    線上完整資料是 3 件；裁剪後的 fixture 只留到 1 件（無盡之刃）。
    兩者都遠低於門檻 5，斷言不受影響。"""
...
def main() -> None:
    ...
    for stat in sorted(table.priced_stats, key=lambda s: s.name):
        ...
        # 用 !r 印出精確的 float round-trip。若用 .4f，四捨五入誤差會超過
        # 黃金測試的 rel=1e-6 容差，導致貼上後 9/21 個案例假性失敗。
...
Expected: 18 passed（含 parametrize）
...
Expected: 15 passed（12 parametrize + 3）
...
> **後記（2026-08-17）**：此手動掃描已升格為常駐測試
> `tests/architecture/test_layer_dependencies.py`，每次 pytest 都會執行，
> 並補掃 ImportFrom 的 names（`from lolcp import infrastructure` 這種寫法）。
...
先在 `src/lolcp/application/ports.py` 追加 `CacheStore`（Interfaces 節列出但原計畫漏了步驟；參數名必須與 `PatchCache` 一致，Step 4 的簽名測試會逐一比對）：
...
- Create: `src/lolcp/presentation/app_coordinator.py`
- Create: `src/lolcp/main.py`
- Test: `tests/presentation/test_sync_worker.py`
- Test: `tests/presentation/test_app_coordinator.py`
...
> **修訂（Task 19 review）**：原版 main() 有三個缺陷 —— (1) plain closure slot 會在
> worker thread 執行，視窗在非 GUI 執行緒建立（Qt 未定義行為，macOS 會崩潰）；
> (2) 每收到一次 finished 就再開一個視窗；(3) progress 訊號從未接上。
> 修法是新增 `src/lolcp/presentation/app_coordinator.py` 的 `AppCoordinator(QObject)`：
> 活在主執行緒、bound-method slot 取得 queued 傳遞、視窗持久重用、
> 換版本時以 `MainWindow.set_use_cases` 重建注入、首次失敗才 `app.quit()`。
> 生命週期測試在 `tests/presentation/test_app_coordinator.py`（7 個）。
...
Run: `uv run pytest tests/presentation/test_sync_worker.py tests/presentation/test_app_coordinator.py tests/test_composition_root.py -v`
Expected: 15 passed（4 + 7 + 4；分層掃描以 `__file__` 錨定絕對路徑並斷言掃描數 > 0，避免 cwd 不同時空轉）

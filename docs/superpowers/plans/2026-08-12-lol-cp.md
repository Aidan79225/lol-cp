...
**尚未覆蓋（屬批次三）**：§6 資料管線與快取（Task 14–16）、§8 介面（Task 17–19）、§10.4 契約測試（Task 15）、§10.5 presentation 測試（Task 17）。
...
**已知待補**：Task 14–19 的逐步驟內容（批次三）。Task 1–13 構成完整的計算閉環——執行完即可用 Python 直接算出任何裝備在任何英雄視角下的兩種 CP值，只是還沒有介面與自動更新。

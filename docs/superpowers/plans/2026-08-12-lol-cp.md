...
- Produces:
  - `NetworkUnavailableError(RuntimeError)`
  ...
  - `PatchGateway` — `Protocol`（application/ports.py）：`latest_version() -> str`、`download_patch(version: str, dest: Path, on_progress) -> None`
  - `HttpPatchGateway(fetcher: HttpFetcher)` — 實作上述 Protocol
...
from lolcp.infrastructure.http.fetcher import HttpFetcher, NetworkUnavailableError
...
- [ ] **Step 5: 在 `src/lolcp/application/ports.py` 追加 `PatchGateway`**
...
**Interfaces:**
- Consumes: `PatchGateway`（Task 15）；`PatchCache`（Task 14）；`NetworkUnavailableError`（Task 15）
- Produces:
  - `SyncResult` — `@dataclass(frozen=True)`：`version: str`、`offline: bool`、`downloaded: bool`
  - `NoDataAvailableError(RuntimeError)`
  - `SyncGameData(gateway: PatchGateway, cache: PatchCache)`
  - `SyncGameData.execute(on_progress: Callable[[int, int], None] | None = None) -> SyncResult`

**分層要求**：本 use case **不得認識 Qt**。進度以普通 `Callable[[int, int], None]` 回傳，由 presentation 層的 worker 轉成 Qt signal。
...
from lolcp.infrastructure.http.fetcher import NetworkUnavailableError
...
本模組不得 import Qt。進度以普通 Callable 回傳，
由 presentation 層的 worker 轉成 Qt signal。有測試守著。
"""
...
from lolcp.application.ports import PatchGateway
from lolcp.infrastructure.cache.patch_cache import PatchCache
from lolcp.infrastructure.http.fetcher import NetworkUnavailableError
...
> `SyncGameData` 位於 application 卻 import `PatchCache`（infrastructure）。
> 這是刻意的：快取是這個 use case 的核心語意而非可替換的細節，
> 抽成 port 只會多一層無人實作第二版的介面。Task 13 的 AST 掃描因此
> 只檢查 `presentation`，不檢查 `infrastructure`——見該步驟的 `forbidden` 設定。
...
- [ ] **Step 5: 修正 Task 13 的 AST 掃描以反映上述例外**

把 Task 13 Step 6 腳本中 application 的 `forbidden` 由 `("infrastructure", "presentation")` 改為 `("presentation",)`，並在腳本頂端加註解說明原因：

```python
    # application 允許 import infrastructure 的 PatchCache（見 Task 16 說明）；
    # 但絕不可 import presentation。
    ("application", ("presentation",)),
```

Run: `uv run pytest -v` — 確認全部通過。
...
有測試以 AST 斷言本模組不 import Qt —— 15.8 MB 下載不可阻塞
UI 執行緒，但 use case 本身不該認識 Qt，進度用普通 Callable。"
```
...
1. **`SyncGameData`（application）import `PatchCache`（infrastructure）是刻意的例外。** 快取是這個 use case 的核心語意，不是可替換的細節；抽成 port 只會多一層永遠只有一個實作的介面。因此 Task 13 Step 6 的 AST 掃描在 Task 16 Step 5 被修正為只禁止 application → presentation。**執行時務必依序完成 Task 16 Step 5，否則 Task 13 的掃描會在 Task 16 之後開始失敗。**

...
**雲陶狂箭「視為疊滿」是一個已知且刻意保留的高估（2026-09-16 記錄）。**
資料實測：`CritPerStackMelee` 0.4%／層、遠程 ×0.5 → **遠程需 125 下普攻、近戰 62 下**
才疊滿 25%（攻速 1.5 時約 83 秒連續普攻）。剛買下的那一刻其實是 0%。

影響已量化（fixture 敏感度實測）：

| 疊層假設 | 達瑞文首件 | 煞蜜拉首件 | 煞蜜拉總攻速 |
|---|---|---|---|
| 疊滿（現行） | 雲陶狂箭 | 雲陶狂箭 | 1.78 |
| 半數 | 海妖殺手 | 蒐集者 | 1.59 |
| 不計 | 海妖殺手 | 蒐集者 | 1.59 |

**這個假設直接決定兩隻英雄的開局件**，不是無害的細節。使用者 2026-09-16 選擇維持疊滿
（該被動是全場永久累積，後期本來就滿）；代價是「第一件」的建議偏向它。
改動此假設前請先重跑敏感度測試。

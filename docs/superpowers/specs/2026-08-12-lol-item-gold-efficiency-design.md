...
- 無地圖限制
- **完全不存法力** — 女神之淚(3070)、藍水晶(1027) 在 bin 中僅有 `maxStack`
...
| 比對項 | 結果 |
|---|---|
| 只有 Data Dragon 有的屬性 | `FlatMPPoolMod`（法力），23 件 SR 裝備 |
...
**生命偷取為扣除錨**：無純吸血基礎裝備，由吸血鬼權杖扣除物攻餘額歸吸血
（(900 − 15AD×35) / 7% = 375/7 ≈ 53.571 每 1%），機制與驗證見
`2026-08-15-deduction-anchors-design.md`。

**其餘 9 種無錨可定價**，介面標示「未定價」，**不當作 0 併入**：

暴擊傷害、固定生命回復、移速%、治療與護盾力量、韌性、緩速抗性、固定魔穿、百分比魔穿、百分比物穿。
...
NNLS condition number   = 733.2    （實測，良態；過高才需警告整欄不可信）
NNLS 低信賴屬性（<5 件）= 6 種：暴擊傷害 1、固定生命回復 2、緩速抗性 3、
                                百分比物穿 4、固定魔穿 4、百分比魔穿 4
SR 屬性種類 = 21（12 可定價 / 9 未定價；生命偷取為扣除錨）
真英雄 = 173              Jade_ 變體 = 60
...
鞋子 1001   治療寶珠 1006   吸血鬼權杖 1053（扣除錨）
```

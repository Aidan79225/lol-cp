...
- 出裝列中可規劃的件（`is_legendary ∨ is_boots`），依原順序成為固定前綴
  —— 以判準而非「是否在傳入的 pool 內」判定（實作時更正：否則滿 6 件前綴
  搭配空 pool 會被整段丟棄）；pool 也由 planner 自行過濾，呼叫端可直接給全部裝備
- 不可規劃的件（部件，如靈巧披風）**不參與規劃**，結果中列出被略過的件
  —— 絕不靜默丟棄
...
**黃金快照**（16.15.1 fixture 實測，β=0.25、空前綴）—— 改版後推薦悄悄改變時，
這是唯一的警報：

```
達瑞文 vs 脆皮：無盡之刃 3031 → 狂戰士護脛 3006 → 幻影之舞 3046 → 狂暴利刃 3097
                → 多明尼克的問候 3036 → 嗜血者 3072          V ≈ 6,778,975
達瑞文 vs 坦克：無盡之刃 3031 → 狂戰士護脛 3006 → 多明尼克的問候 3036 → 幻影之舞 3046
                → 狂暴利刃 3097 → 嗜血者 3072                V ≈ 4,040,773
```

同一組六件，打坦克時多明尼克（百分比物穿）從第 5 件提前到第 3 件 ——
與邊際效益 spec §8「打坦買 LDR」同一條數學，這次由購買順序自己排出來。

**2026-09-15 更新（裝備被動上線，`2026-09-15-item-passives-design.md`）** ——
上表為無被動模型的快照，已被取代。含 22 件被動後（16.15.1 與 16.16.1 相同）：

```
達瑞文 vs 脆皮：海妖殺手 6672 → 狂戰士護脛 3006 → 鬼索的狂暴之刃 3124 → 臨界點 3302
                → 雲陶狂箭 3032 → 無盡之刃 3031
達瑞文 vs 坦克：殞落王者之劍 3153 → 狂戰士護脛 3006 → 臨界點 3302 → 鬼索的狂暴之刃 3124
                → 雲陶狂箭 3032 → 無盡之刃 3031
```

- 推薦從純屬性裝（幻影之舞、狂暴利刃、嗜血者）轉向命中特效裝 —— 被動價值終於被算到
- 打坦克開局件從海妖換成破敗（吃當前生命）；臨界點（雙穿 30%）取代多明尼克的位置
- 煞蜜拉仍與達瑞文完全相同；凱爾僅打坦克時鬼索與臨界點對調 —— 英雄差異要等技能模型
（與 §9 原型在 16.16.1 的序列不同：原型只重排最佳的一個集合，正式版對
beam 內每個集合都窮舉排列。）
...
1. 裝備被動 —— 與三隻英雄相關的約 15–20 件；數值讀 bin `mDataValues`
   ...
   （2026-09-15 立案：`2026-09-15-item-passives-design.md`）

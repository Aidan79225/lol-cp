...
實測（2026-09-15，16.16.1）：接上技能後單次 **1.8–2.8 秒**，超標。實際採用的優化
（皆不改變結果，六組序列優化前後逐一比對相同）：

1. **規劃器評分依件集合快取** —— 評分只取決於「哪些件」與件數、與順序無關；
   第 2 階段 5 件的 120 種排列只有 32 種前綴集合。這是主要收益
2. `skill_ranks` 以 `lru_cache` 快取（輸入僅 18 × 6 種）
3. `SpellData.values_at` 依技能等級快取
4. 被動與技能套用時，公式屬性表每次套用只取一次快照

優化後單次 **0.61–0.82 秒**；全套測試由 34 秒回到 15 秒。
